  ...
  events: [
    {
      ...
      async execute(interaction: Interaction) {
        ...
        const board = interaction.message.components!.map((r) =>
          r.components.map((x) => parseInt((x as MARC).customId!.split("-")[3]))
        ) as TTTBoard;
          ...
          await interaction.reply({
            embeds: [
              MGEmbed(MGStatus.Error)
                ...
                .setDescription("...wait, how did you even trigger this?"),
            ],
            ephemeral: true,
          });
          return;
        ...
        if (!(sel_val === 4 && (p1id === interaction.user.id) === p1turnp)) {
          await interaction.reply({
            embeds: [
              MGEmbed(MGStatus.Info).setTitle("It's not your turn yet..."),
            ],
            ephemeral: true,
          });
          return;
...
function check_gameover(board: TTTBoard): boolean {
  return check_win(board, true) || check_win(board, false) || check_draw(board);
}
    ...
    new MessageActionRow().setComponents(
      ...a.map((x, col) =>
        new MessageButton()
          ...
          .setStyle(x === 0 ? "DANGER" : x === 1 ? "SUCCESS" : "PRIMARY")
...
function gen_disc_embed(
  ...
  if (check_draw(board)) {
    ...
    return MGEmbed(MGStatus.Success)
      .setTitle("Game over!")
      .setDescription(`Congratulations, ${p1ping}, you won! Rating +100.`);
  } else if (check_win(board, false)) {
    return MGEmbed(MGStatus.Success)
      .setTitle("Game over!")
      .setDescription(`Congratulations, ${p2ping}, you won! Rating +100.`);
  }

  return MGEmbed(MGStatus.Info)
    .setTitle("Tic-tac-toe")
    .setDescription(
      `${p1ping} vs ${p2ping}... glhf! ${p1turnp ? p1ping : p2ping}'s turn.`
    );
}
