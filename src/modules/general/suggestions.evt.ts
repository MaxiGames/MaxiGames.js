    ...
    async execute(interaction: ButtonInteraction) {
      ...
      if (interaction.customId.endsWith("suggestions")) {
        if (
          ...
          const suggestion = message.embeds[0].description;
          const author = message.embeds[0]
            .title!.split("(")[1]
            .replace(")", "")!;
          const user = interaction.client.users.cache.get(author)!;
          await interaction.reply({
          ...
          message.channel
            ...
            .then(async (messages) => {
              const reason = messages.first()!.content;
              const success = interaction.customId.startsWith("Accept");
              let added = 0;

              if (success) {
                added = Math.ceil(Math.random() * 100);
                const data = await MGFirebase.getData(`user/${author}`);
                data["money"] += added;
              ...
              await channel.send({
                embeds: [
                  MGEmbed(success ? MGStatus.Success : MGStatus.Error)
                    .setTitle(
                      success ? "Suggestion Accepted" : "Suggestion Rejected"
                    )
